...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < eCount; i++)
    {
        ...
        test.vList.at(src).isActive = true;
        test.vList.at(dst).isActive = true;
    ...
    for(int i = 0; i < nodeCount; i++)
        clientVec.push_back(UtilClient<LPA_Value, LPA_MSG>(vCount, ((i + 1) * eCount) / nodeCount - (i * eCount) / nodeCount, numOfInitV, i, 1000));
    int chk = 0;
